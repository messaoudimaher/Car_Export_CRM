...
async def scan_and_notify_due_followups(ctx: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    - Interval: Scheduled via ARQ cron/worker every 60 seconds.
    - Concurrency: Uses row-level locking `with_for_update(skip_locked=True)` so concurrent worker
      replicas never claim or double-notify the same due follow-up record.
    - Downtime Recovery: Overdue reminders (`due_at <= NOW()`) accumulated during worker downtime or
      deployments are automatically scanned and dispatched upon worker startup.
    - WebSocket Resilience: Transient WebSocket delivery failure logs a warning; users can recover
      missed reminders via `GET /api/v1/followups?due_before=...`.
    - Tenant Authorization: Data payload is strictly scoped to tenant entities (SEC-007).
    ...
    async with async_session_factory() as session:
        ...
        try:
            ...
            # Fallback for SQLite in unit tests which doesn't support SELECT ... FOR UPDATE
        ...
        for task in due_tasks:
            ...
            try:
                ...
                logger.warning(
                    f"WebSocket broadcast failed for follow-up '{task.id}': {ws_err}",
                    extra={"followup_id": str(task.id), "tenant_id": str(task.tenant_id)},
                )

            # Persist reminder_sent status regardless to ensure worker idempotency
